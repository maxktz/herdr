...
pub(crate) fn pane_border_title(label: &str, pane_width: u16, focused: bool) -> Option<String> {
...
fn layout_panes_for_chrome(
    layout: &crate::layout::TileLayout,
    area: Rect,
    shared_pane_borders: bool,
) -> Vec<PaneInfo> {
    if shared_pane_borders {
        layout.panes_with_frame_layout(area, PaneFrameLayout::Shared)
    } else {
        layout.panes(area)
    }
}
...
pub(super) fn resize_tab_panes(
    ...
    for info in apply_pane_chrome(
        layout_panes_for_chrome(&tab.layout, area, app.shared_pane_borders),
        app.pane_borders,
        app.pane_gaps && !app.shared_pane_borders,
    ) {
...
pub(super) fn compute_pane_infos(
    ...
    let mut pane_infos = apply_pane_chrome(
        layout_panes_for_chrome(&ws.layout, area, app.shared_pane_borders),
        app.pane_borders,
        app.pane_gaps && !app.shared_pane_borders,
    );
...
mod tests {
    ...
    fn pane_border_renderer_places_adjacent_cjk_by_display_width() {
        ...
        app.view.pane_infos[0].id = pane_id;
