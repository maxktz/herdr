...
Direct pane-focus keys use smart Vim/Neovim routing by default. When the focused pane's foreground process matches `[terminal] smart_pane_navigation_processes`, Herdr forwards direct `focus_pane_left`, `focus_pane_down`, `focus_pane_up`, and `focus_pane_right` keys into the pane instead of changing Herdr focus. An editor-side mapping can then move inside editor splits first and call `herdr pane focus --direction ...` only when the editor is already at an edge.
...
`separator` controls structural pane frames and major sidebar dividers independently from dim surfaces and scrollbar tracks. Color values accept hex, named colors, `rgb(r,g,b)`, or reset aliases like `reset`, `default`, `none`, and `transparent`.

Named external themes can provide a complete palette in `themes/<name>.toml` next to `config.toml`. Set `theme.name` to the filename without `.toml`. Valid external themes also appear in the Settings theme palette. External theme files use `schema_version = 1`, a matching `name`, and a `[colors]` table containing every theme token. Built-in themes keep priority over files with the same name.
...
`confirm_close` controls whether closing a workspace asks for confirmation. `prompt_new_tab_name` controls whether new tabs ask for a label first. Set `dim_auto_named_tabs = false` to render generated and manually named tabs with the same higher-contrast text styles.

Set `show_pane_scrollbars = false` to hide pane scrollbars and return their reserved right-hand column to pane applications. Scrolling through pane history still works with the mouse wheel and copy-mode controls.

Set `tab_bar_left_padding = true` to retain one blank column before desktop tab controls.

The expanded desktop sidebar can be compacted independently: `show_sidebar_quit_button = true` adds a traffic-light button that performs the configured detach action, `show_sidebar_section_labels = false` removes the section labels while retaining one row of top padding, `agent_sort_toggle_in_footer = true` moves the grouped/priority control to the bottom-left without bold styling, `show_agent_sort_toggle = false` hides that control while preserving the configured `agent_panel_sort`, `inset_sidebar_collapse_button = true` moves the collapse icon inward, `sidebar_action_icons = true` replaces the new-space and menu labels with icons, and `inset_sidebar_menu_button = true` moves the menu icon inward. The collapsed and mobile sidebars are unchanged.

Set `shared_pane_borders = true` to draw one separator cell between adjacent panes while retaining the outer frame and a complete ring around each pane. On desktop layouts with multiple panes, the leftmost pane also shares the sidebar divider instead of drawing a duplicate border beside it. The focused pane owns the color and style of shared border cells. Single-pane tabs keep the standalone sidebar divider and no pane frame. Set `thick_focused_pane_border = false` to keep the focused ring one cell wide instead of using thick line glyphs. Both options are also available in the panes tab of in-app settings.
