---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Cursor Agent CLI, and Neovim.
---
...
herdr integration uninstall neovim
```
...
Neovim integration is separate from agent session restore. It installs a small navigator plugin that moves between Neovim splits first and calls `herdr pane focus --direction ...` when the editor is already at a split edge.
...
Install the Neovim navigator plugin:

```bash
herdr integration install neovim
```

Herdr writes the bundled plugin to:

```text
~/.config/nvim/plugin/herdr-navigator.lua
```

If `XDG_CONFIG_HOME` is set, Herdr writes to `$XDG_CONFIG_HOME/nvim/plugin/herdr-navigator.lua` instead. The Neovim config directory must already exist. Uninstall removes only that plugin file.

The plugin maps `<C-h>`, `<C-j>`, `<C-k>`, and `<C-l>` in normal mode. Each mapping first moves between Neovim splits. If the current window does not change and the editor is running inside a Herdr pane, it calls `herdr pane focus --direction left|down|up|right` for the current pane.

Set `vim.g.herdr_navigator_no_mappings = 1` before the plugin loads if you want to keep the user commands but define your own mappings.
